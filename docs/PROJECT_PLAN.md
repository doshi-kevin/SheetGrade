...
- **No large local LLM.** Free API tiers for generation. Local models limited to embeddings (~0.5 GB) and optionally a reranker (~0.6 GB) on the 5070.
- **SQLite + `sqlite-vec`** for the misconception store at first — zero infrastructure. Move to Qdrant only if you outgrow it.
...
# Phase B — Understanding a sheet (parts 5–8)
...
### Part 8 — Region role classification
...
**Learn:** first place a small LLM earns its keep — and where a rules baseline nearly matches it.

---

# Phase C — Alignment (parts 9–12) ← the moat

### Part 9 — Sheet matching
...
### Part 12 — Confidence and abstention
...
**Learn:** in grading, a confident wrong answer is far worse than "I need a human." This is the feature that makes professors trust it.

---

# Phase D — Comparison and scoring (parts 13–16)

### Part 13 — Typed comparators
...
### Part 14 — Formatting and presentation checks
...
### Part 15 — Rubric model
**Build:** a rubric DSL (YAML) binding point values to region roles and checks, plus an LLM-assisted importer that turns a professor's prose rubric into a draft DSL for human confirmation.
...
### Part 16 — Scoring with carry-through
...
# Phase E — Intelligence (parts 17–19)

### Part 17 — Misconception library
...
| Topic | Part |
|---|---|
| Embeddings, semantic search | 7, 17 |
| Vector store, indexing, similarity search | 17 |
| Classification, clustering, anomaly detection | 8, 19 |
| RAG (retrieval over misconceptions + course material) | 17, 18 |
| Chunking / retrieval unit design | 17 |
| Structured outputs, schema-constrained decoding | 8, 15, 18 |
| System / role / constraint prompting | 15, 18 |
| Few-shot (retrieved graded exemplars) | 18 |
| Context engineering: isolation, trust boundaries | 18 |
| Prompt injection defence | 18 |
| Evaluation, LLM-as-judge, faithfulness | 4, 18 |
| Sampling parameters, determinism | 13, 18 |
| Function calling | 20 |

Agents, MCP and multi-agent are deliberately excluded. Part 20's API is already a set of agent tools; wrapping it in MCP is the on-ramp to the next project.

---

## Order of attack, given limited time

Parts 1–4 are non-negotiable and unglamorous. Do not skip to part 10 because alignment is the interesting bit — without the mutation engine you'll have no way to know if your alignment works, and you'll burn far more time debugging by intuition than you saved.

Parts 9–12 are where you should spend the most thinking time and the least Claude Code time.

Parts 17–19 are the ones to cut or defer if time runs short. Parts 1–16 alone are already a better product than what exists.